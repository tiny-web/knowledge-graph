...
# Attach Policy to IAM Role
resource "aws_iam_role_policy" "register_app_policy" {
  name = "register_app_policy"
  role = aws_iam_role.register_app_role.id
...
# Lambda Function
data "archive_file" "register_app_zip" {
  type        = "zip"
  source_dir  = "${path.module}/src"
  output_path = "${path.module}/src.zip"
}

resource "aws_lambda_function" "register_app" {
  function_name    = var.lambda_name
  role             = aws_iam_role.register_app_role.arn
  handler          = "index.handler"
  runtime          = "nodejs20.x"
  filename         = data.archive_file.register_app_zip.output_path
  source_code_hash = data.archive_file.register_app_zip.output_base64sha256
  timeout          = 300 # 5 mins
  environment {
    variables = {
      KG_APP_CONFIG_TABLE_NAME = var.kg_app_config_table_name
      KG_APP_CONFIG_TABLE_ARN = var.kg_app_config_table_arn
    }
  }
...
resource "aws_api_gateway_rest_api" "query_api" {
  name        = var.api_name
  description = "REGISTER app KG API Gateway"
}
...
resource "aws_api_gateway_integration" "lambda_integration" {
  ...
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.register_app.invoke_arn
}

resource "aws_lambda_permission" "api_gateway_permission" {
  statement_id  = "AllowAPIGatewayInvoke"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.register_app.function_name
...
resource "aws_api_gateway_integration_response" "kg_register_post_200" {
  rest_api_id = aws_api_gateway_rest_api.query_api.id
  resource_id = aws_api_gateway_resource.query_resource.id
  http_method = aws_api_gateway_method.post_method.http_method
...
resource "aws_api_gateway_integration_response" "kg_app_register_options_200" {
  rest_api_id = aws_api_gateway_rest_api.query_api.id
  resource_id = aws_api_gateway_resource.query_resource.id
  http_method = aws_api_gateway_method.options.http_method
...
resource "aws_api_gateway_deployment" "query_api_deployment" {
  ...
  depends_on = [
    ...
    aws_api_gateway_integration_response.kg_register_post_200,
    aws_api_gateway_method_response.options_200,
    aws_api_gateway_integration_response.kg_app_register_options_200
  ]
}
